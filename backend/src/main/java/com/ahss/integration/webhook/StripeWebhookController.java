...
public class StripeWebhookController extends BaseWebhookController {
    ...
    public ResponseEntity<ApiResponse<Void>> handleStripeWebhook(
        ...
        return handleWebhook(body, headers, "/api/integrations/webhooks/stripe", "Stripe webhook accepted");
    }

    @Override
    protected String gatewayName() { return "Stripe"; }

    @Override
    protected String extractEventType(JsonNode root) { return text(root.get("type")); }

    @Override
    protected PaymentCallbackType mapEventType(String eventType) { return WebhookEventTypeMapper.mapStripe(eventType); }
    ...
    // Common helpers provided by BaseWebhookController
}