...
public class PaymentSagaOrchestrator {
    ...
    private void handleRequestApproval(PaymentCallbackEvent event) {
        ...
        PaymentRequestDto updated = requestService.updateStatus(requestId, PaymentRequestStatus.APPROVED, "Approved via callback: " + safe(event.getGatewayName()));
        auditLogService.logPaymentRequestAction(requestId, "REQUEST_APPROVED", null, PaymentRequestStatus.APPROVED.toString(), "Request approved by vendor", null, null, null, null);
        eventProducer.send(new PaymentDomainEvent("request.approved", event.getCorrelationId(), Map.of("requestId", updated.getId())));
    }
    ...
    private void handleRequestRejection(PaymentCallbackEvent event) {
        ...
        PaymentRequestDto updated = requestService.updateStatus(requestId, PaymentRequestStatus.REJECTED, reasonFrom(event));
        auditLogService.logPaymentRequestAction(requestId, "REQUEST_REJECTED", null, PaymentRequestStatus.REJECTED.toString(), "Request rejected by vendor", null, null, null, null);
        eventProducer.send(new PaymentDomainEvent("request.rejected", event.getCorrelationId(), Map.of("requestId", updated.getId())));
    }

    private void handlePaymentSuccess(PaymentCallbackEvent event) {
        Optional<PaymentTransactionDto> opt = transactionService.getTransactionByExternalId(event.getExternalTransactionId());
        if (opt.isEmpty()) {
        ...
        requestService.markAsPaid(tx.getPaymentRequestId(), event.getReceivedAt() != null ? event.getReceivedAt() : LocalDateTime.now());
        auditLogService.logTransactionAction(tx.getId(), "PAYMENT_SUCCESS", PaymentTransactionStatus.PROCESSING.toString(), PaymentTransactionStatus.SUCCESS.toString(), "Payment processed successfully", null, null, null, null);
        eventProducer.send(new PaymentDomainEvent("payment.success", event.getCorrelationId(), Map.of("transactionId", tx.getId())));
    }
    ...
    private void handlePaymentFailure(PaymentCallbackEvent event) {
        ...
        auditLogService.logTransactionAction(tx.getId(), "PAYMENT_FAILED", PaymentTransactionStatus.PROCESSING.toString(), PaymentTransactionStatus.FAILED.toString(), reasonFrom(event), null, null, null, null);
        eventProducer.send(new PaymentDomainEvent("payment.failed", event.getCorrelationId(), Map.of("transactionId", tx.getId())));
    }
    ...
    private void handleRefundSuccess(PaymentCallbackEvent event) {
        ...
        if (txOpt.isPresent()) {
            ...
            PaymentRequestStatus newStatus = txAmount != null && refundAmount != null && refundAmount.compareTo(txAmount) >= 0
                    ? PaymentRequestStatus.REFUNDED
                    : PaymentRequestStatus.PARTIAL_REFUND;
            requestService.updateStatus(txOpt.get().getPaymentRequestId(), newStatus, "Refund processed: " + safe(event.getGatewayName()));
        }
        auditLogService.logRefundAction(refund.getId(), "REFUND_SUCCESS", PaymentTransactionStatus.PROCESSING.toString(), PaymentTransactionStatus.SUCCESS.toString(), "Refund processed successfully", null, null, null, null);
        eventProducer.send(new PaymentDomainEvent("refund.success", event.getCorrelationId(), Map.of("refundId", refund.getId())));
    }
    ...
    private void handleRefundFailure(PaymentCallbackEvent event) {
        ...
        auditLogService.logRefundAction(refund.getId(), "REFUND_FAILED", PaymentTransactionStatus.PROCESSING.toString(), PaymentTransactionStatus.FAILED.toString(), reasonFrom(event), null, null, null, null);
        eventProducer.send(new PaymentDomainEvent("refund.failed", event.getCorrelationId(), Map.of("refundId", refund.getId())));
    }

    private UUID resolveRequestId(PaymentCallbackEvent event) {
        if (event.getPaymentRequestId() != null) return event.getPaymentRequestId();
        if (event.getPaymentToken() != null) {
            Optional<PaymentRequestDto> pr = requestService.getPaymentRequestByToken(event.getPaymentToken());
            if (pr.isPresent()) return pr.get().getId();
        }
    ...
    private String reasonFrom(PaymentCallbackEvent e) { return safe(e.getErrorCode()) + (e.getErrorMessage() != null ? (": " + e.getErrorMessage()) : ""); }
}