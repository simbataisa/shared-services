...
class TenantControllerTest {
    ...
    void get_tenant_by_code_not_found_returns_404() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/tenants/code/NOPE"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success", is(true)))
                .andExpect(jsonPath("$.message", is("Tenant not found")))
    ...
    void update_tenant_not_found_returns_404() throws Exception {
        when(tenantService.updateTenant(eq(77L), any(TenantDto.class))).thenThrow(new RuntimeException("Tenant not found with id: 77"));
        TenantDto dto = new TenantDto();
        dto.setName("ACME");
        dto.setCode("ACME");
        String body = objectMapper.writeValueAsString(dto);
        mockMvc.perform(put("/api/v1/tenants/77")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isNotFound())
    ...
    void update_tenant_bad_request_returns_400() throws Exception {
        when(tenantService.updateTenant(eq(78L), any(TenantDto.class))).thenThrow(new RuntimeException("Invalid data"));
        TenantDto dto = new TenantDto();
        dto.setName("ACME");
        dto.setCode("ACME");
        String body = objectMapper.writeValueAsString(dto);
        mockMvc.perform(put("/api/v1/tenants/78")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success", is(false)))
    ...
    void delete_tenant_not_found_returns_404() throws Exception {
        doThrow(new RuntimeException("Tenant not found with id: 90")).when(tenantService).deleteTenant(90L);

        mockMvc.perform(delete("/api/v1/tenants/90"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success", is(false)))
                .andExpect(jsonPath("$.message", containsString("Tenant not found")))
    ...
    void update_status_missing_status_returns_400() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/tenants/5/status")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success", is(false)))
    ...
    void update_status_not_found_returns_404() throws Exception {
        ...
        doThrow(new RuntimeException("Tenant not found with id: 5")).when(tenantService).activateTenant(5L);
    ...
    void activate_deactivate_suspend_not_found_return_404() throws Exception {
        doThrow(new RuntimeException("Tenant not found with id: 11")).when(tenantService).activateTenant(11L);
        doThrow(new RuntimeException("Tenant not found with id: 12")).when(tenantService).deactivateTenant(12L);
        doThrow(new RuntimeException("Tenant not found with id: 13")).when(tenantService).suspendTenant(13L);

        mockMvc.perform(patch("/api/v1/tenants/11/activate"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success", is(false)))
                .andExpect(jsonPath("$.path", is("/api/v1/tenants/11/activate")));

        mockMvc.perform(patch("/api/v1/tenants/12/deactivate"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success", is(false)))
                .andExpect(jsonPath("$.path", is("/api/v1/tenants/12/deactivate")));

        mockMvc.perform(patch("/api/v1/tenants/13/suspend"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success", is(false)))
