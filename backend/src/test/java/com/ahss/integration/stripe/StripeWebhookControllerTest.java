...
class StripeWebhookControllerTest extends BaseIntegrationTest {
    ...
    void handle_stripe_payment_intent_failed_with_last_error() {
        ...
        try { Allure.addAttachment("Stripe Event Payload (Failed)", "application/json", new ObjectMapper().writeValueAsString(evt)); } catch (Exception ignored) {}
    ...
    void handle_stripe_charge_refunded_sets_refund_id() {
        ...
        verify(producer, times(1)).send(captor.capture());
        PaymentCallbackEvent evt = captor.getValue();
        try { Allure.addAttachment("Stripe Event Payload (Refund)", "application/json", new ObjectMapper().writeValueAsString(evt)); } catch (Exception ignored) {}
