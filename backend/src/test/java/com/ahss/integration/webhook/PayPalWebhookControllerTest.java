...
class PayPalWebhookControllerTest extends BaseIntegrationTest {
    ...
    void handle_paypal_payment_capture_completed_sends_event() {
        ...
        try { Allure.addAttachment("PayPal Event Payload", "application/json", new ObjectMapper().writeValueAsString(evt)); } catch (Exception ignored) {}
    ...
    void handle_paypal_payment_capture_denied_sets_error() {
        ...
        verify(producer, times(1)).send(captor.capture());
        PaymentCallbackEvent evt = captor.getValue();
        try { Allure.addAttachment("PayPal Event Payload (Denied)", "application/json", new ObjectMapper().writeValueAsString(evt)); } catch (Exception ignored) {}
    ...
    void handle_paypal_payment_capture_refunded_no_amount_branch() {
        ...
        verify(producer, times(1)).send(captor.capture());
        PaymentCallbackEvent evt = captor.getValue();
        try { Allure.addAttachment("PayPal Event Payload (Refunded)", "application/json", new ObjectMapper().writeValueAsString(evt)); } catch (Exception ignored) {}
