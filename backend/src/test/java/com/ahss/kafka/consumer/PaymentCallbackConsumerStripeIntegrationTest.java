...
public class PaymentCallbackConsumerStripeIntegrationTest {
        ...
        @Autowired
        private PaymentSagaOrchestrator orchestrator; // ensure real orchestrator runs
        ...
        @MockBean
        private com.ahss.kafka.producer.PaymentEventProducer eventProducer;
        ...
        void stripe_payment_intent_succeeded_triggers_processing() {
                ...
                String stripePayload = Allure.step("Create Stripe Payment Intent Succeeded Event", () -> "{" +
                                "\"id\":\"evt_123\"," +
                                "\"type\":\"payment_intent.succeeded\"," +
                                "\"data\":{" +
                                "  \"object\": {\"id\": \"" + externalTxId
                ...
                PaymentTransactionDto txDto = Allure.step("Create Payment Transaction DTO",
                                () -> new PaymentTransactionDto());
                txDto.setId(txId);
                txDto.setPaymentRequestId(reqId);
                Allure.step("Mock Transaction Service to return Payment Transaction DTO",
                                () -> when(transactionService.getTransactionByExternalId(eq(externalTxId)))
                                                .thenReturn(Optional.of(txDto)));

                Allure.step("Consume Stripe Payment Intent Succeeded Event", () -> consumer.onMessage(stripePayload));

                Allure.step("Verify Transaction Service is called to get transaction by external ID",
                                () -> verify(transactionService, times(1))
                                                .getTransactionByExternalId(eq(externalTxId)));
                Allure.step("Verify Transaction Service is called to mark transaction as processed",
                                () -> verify(transactionService, times(1)).markAsProcessed(eq(txId), eq(externalTxId),
                                                any(Map.class)));
                Allure.step("Verify Request Service is called to mark request as paid",
                                () -> verify(requestService, times(1)).markAsPaid(eq(reqId), any(LocalDateTime.class)));
                Allure.step("Verify Audit Log Service is called to log transaction action",
                                () -> verify(auditLogService, times(1)).logTransactionAction(eq(txId),
                                                eq("PAYMENT_SUCCESS"),
                                                anyString(), anyString(), anyString(), any(), any(), any(), any()));

                Allure.step("Verify Event Producer is called to send payment success event", () -> {
                        ArgumentCaptor<PaymentDomainEvent> eventCaptor = ArgumentCaptor
                                        .forClass(PaymentDomainEvent.class);
