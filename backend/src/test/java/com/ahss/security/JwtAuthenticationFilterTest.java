...
class JwtAuthenticationFilterTest {
    ...
    void valid_token_populates_security_context_with_authorities() throws ServletException, IOException {
        String token = tokenWithAdminRole();
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/v1/payments");
        request.addHeader("Authorization", "Bearer " + token);
        Allure.addAttachment("Authorization Header", MediaType.TEXT_PLAIN_VALUE, request.getHeader("Authorization"));
        MockHttpServletResponse response = new MockHttpServletResponse();
        RecordingChain chain = new RecordingChain();

        Allure.step("Execute filter chain", () -> {
            try {
        ...
        assertEquals("admin@ahss.com", auth.getPrincipal());
        assertTrue(auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_SYSTEM_ADMINISTRATOR")));
    }

    @Test
    @Story("Invalid token does not authenticate")
    @Severity(SeverityLevel.NORMAL)
    void invalid_token_does_not_authenticate_but_chain_proceeds() throws ServletException, IOException {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/v1/payments");
