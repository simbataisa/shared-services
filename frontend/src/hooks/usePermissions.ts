...
export const usePermissions = () => {
  const { user, hasPermission, hasRole, hasAnyRole, canAccessResource } = useAuth()
  ...
  const canManageModules = canCreateModules || canUpdateModules || canDeleteModules
  ...
  const canManageTenantUsers = isTenantAdmin || hasPermission('tenant_user:manage')
...
export const useCrudPermissions = (resource: string) => {
  ...
  return {
    ...
    canManage: hasPermission(`${resource}:create`) || 
