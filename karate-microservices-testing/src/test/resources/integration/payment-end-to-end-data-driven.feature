Feature: End-to-end payment success flow - Data Driven
  ...
  Scenario Outline: <testCase> - Create payment request with <paymentMethod> via <gateway>
    ...
    * def createResult = call read('classpath:integration/helpers/create-payment-request.feature') { allowedPaymentMethods: #(allowedPaymentMethodsArray), preSelectedPaymentMethod: #(preSelectedPaymentMethod), paymentGateway: #(paymentGateway), auth: #(auth), headers: #(headersPreview) }
    ...
    * def processResult = call read('classpath:integration/helpers/process-payment-transaction.feature') { paymentToken: #(paymentToken), paymentMethod: '<paymentMethod>', gatewayName: '<gateway>', paymentMethodDetails: #(paymentMethodDetailsObj), auth: #(auth), headers: #(headersPreview) }
